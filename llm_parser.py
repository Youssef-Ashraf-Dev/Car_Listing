...
class Tires(BaseModel):
    ...
    type: str = Field(
        description="The condition of the tires, e.g., 'brand-new' or 'used'"
    )
...
class Notice(BaseModel):
    ...
    type: str = Field(
        description="The type of notice, e.g., 'collision' or 'small accident'"
    )
...
class Car(BaseModel):
    ...
    body_type: str = Field(
        description="The body type of the car, e.g., 'sedan'. Will be filled later."
    )
    ...
    motor_size_cc: Optional[int] = Field(
        description="The engine size in cubic centimeters (cc)."
    )
    tires: Optional[Tires] = Field(description="Information about the car's tires")
    windows: str = Field(
        description="Description of the car's windows, e.g., 'tinted' or 'electrical'"
    )
    notices: Optional[List[Notice]] = Field(
        description="A list of any notices or issues with the car"
    )
    price: Optional[Price] = Field(
        alias="estimated_price", description="The price of the car"
...
def create_car_data_parser_chain():
    ...
    prompt = ChatPromptTemplate.from_messages(
        [
            (
                "system",
                """You are a specialized AI assistant for parsing car descriptions into structured JSON.
...
if __name__ == "__main__":
    ...
    try:
        ...
        print(
            "This often happens if the input text is too ambiguous or missing critical information."
        )

    except (APIConnectionError, RateLimitError) as e:
        print(f"\n--- ERROR: OpenAI API Connection Issue ---")
        print(
            f"Could not connect to the Azure OpenAI service. Error: {type(e).__name__}"
        )
        print(
            "Please check your network connection and ensure the API endpoint is correct."
        )
        ...
        print(
            "Please verify your `AZURE_OPENAI_API_KEY` and other credentials in the .env file."
        )
